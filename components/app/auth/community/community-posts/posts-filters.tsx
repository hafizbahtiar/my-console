    ...
    <Card className="border-dashed">
      <CardHeader className="pb-4 p-4 sm:p-6">
        <CardTitle className="text-base sm:text-lg flex items-center gap-2" suppressHydrationWarning>
          ...
          {t('community_posts_page.filters.title')}
        </CardTitle>
        <CardDescription className="text-xs sm:text-sm" suppressHydrationWarning>
          {t('community_posts_page.filters.description')}
        </CardDescription>
      </CardHeader>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end">
          <div className="flex-1 space-y-2 min-w-0">
            <label className="text-xs sm:text-sm font-medium text-muted-foreground" suppressHydrationWarning>
              {t('community_posts_page.filters.search_label')}
            </label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground shrink-0" />
              <Input
                type="text"
                placeholder={t('community_posts_page.filters.search_placeholder')}
                value={searchTerm}
          ...
          <div className="space-y-2">
            <label className="text-xs sm:text-sm font-medium text-muted-foreground" suppressHydrationWarning>
              {t('community_posts_page.filters.status_filter')}
            </label>
            <Select value={statusFilter} onValueChange={onStatusFilterChange}>
              <SelectTrigger className="w-full md:w-48">
                <SelectValue placeholder={t('community_posts_page.filters.status_placeholder')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">
                  <div className="flex items-center gap-2">
                    <span suppressHydrationWarning>{t('community_posts_page.filters.all_status')}</span>
                ...
                <SelectItem value="pending">
                  <div className="flex items-center gap-2">
                    ...
                    <span suppressHydrationWarning>{t('pending')}</span>
                    <Badge variant="secondary" className="text-xs">
                ...
                <SelectItem value="approved">
                  <div className="flex items-center gap-2">
                    ...
                    <span suppressHydrationWarning>{t('approved')}</span>
                    <Badge variant="secondary" className="text-xs">
                ...
                <SelectItem value="rejected">
                  <div className="flex items-center gap-2">
                    ...
                    <span suppressHydrationWarning>{t('rejected')}</span>
                    <Badge variant="secondary" className="text-xs">
          ...
          {(searchTerm || statusFilter !== 'all') && (
            ...
            >
              {t('community_posts_page.filters.clear_filters')}
            </Button>
          )}
        </div>
      </CardContent>
